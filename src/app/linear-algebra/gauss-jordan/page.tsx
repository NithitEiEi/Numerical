...
import { gaussJordanElimination } from "@/app/script/linearMethod";
...
export default function Page() {
  ...
  const calculate = () => {
    ...
    const gaussMatrix: number[][] = a.map((row) => row.map(Number));
    for (let i = 0; i < b.length; i++) {
      gaussMatrix[i].push(Number(b[i]));
    }

    const result = gaussJordanElimination(gaussMatrix);
    console.log(result);
    setAnswer(result);
  };
  ...
  return (
    ...
    <title>Gauss Jordan</title>
      <div className="mt-12 flex justify-center items-center">
        <div className="mb-10 p-8 grid grid-cols-12 border-2 border-red-400 w-1/2 gap-4">
          <div className="col-span-12 text-2xl font-bold">Gauss Jordan</div>
          <input
